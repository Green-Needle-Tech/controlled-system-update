#!/usr/bin/env bash
#
# e2e-test.sh — End-to-end test for controlled-system-update v2.2.0
...
echo "=========================================="
echo "E2E Test: controlled-system-update v2.2.0"
...
if [[ "$skill_version" == "2.2.0" ]]; then
    pass "SKILL.md version is 2.2.0"
else
    fail "SKILL.md version is '$skill_version' (expected 2.2.0)"
fi
...
test_msg="[E2E TEST] controlled-system-update v2.2.0 — test notification from $(hostname -s)"
...
if [[ -f "$local_skill" ]]; then
    ...
    if [[ "$local_skill_version" == "2.2.0" ]]; then
        pass "Local skill updated to v2.2.0"
    else
        fail "Local skill version is '$local_skill_version' (expected 2.2.0)"
    fi
else
